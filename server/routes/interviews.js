...
const validateUpdateResult = [
  body('result').isIn(['Pending', 'Passed', 'Failed', 'No Show']).withMessage('Invalid interview result'),
...
const validateCancel = [
  body('reason').optional().isLength({ min: 1 }).withMessage('Reason must not be empty')
];
